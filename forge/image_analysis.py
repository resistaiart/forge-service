...
MODELS = {
    ...
    # 🔥 new tags mode uses blip-base under the hood# forge/image_analysis.py
import requests
import time
import os
import base64
...
MODELS = {
    "basic": "Salesforce/blip-image-captioning-base",
    "detailed": "Salesforce/instructblip-vicuna-7b",
    # 🔥 tags mode uses blip-base under the hood
...
# Simple stopwords for tag cleaning
...
def analyse_image(image_input: Union[str, bytes], caption: Optional[str] = None, mode: str = "basic") -> Dict[str, Any]:
    ...
    try:
        ...
        if mode in {"basic", "tags"}:
            ...
            question = (
                "Describe this image in extreme detail. Include objects, colors, composition, "
                "style, mood, and any text visible."
            )
            if caption:
