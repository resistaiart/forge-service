...
def optimise_i2i_package(
    ...
    diagnostics = _generate_diagnostics(settings, intent, "i2i")
    diagnostics["denoise_reason"] = (
        f"Denoise {denoise_strength} for "
        f"{'strong' if denoise_strength > 0.6 else 'moderate'} transformation"
    )
...
def optimise_i2v_package(
    ...
    diagnostics = _generate_video_diagnostics(settings, intent)
    diagnostics["denoise_reason"] = (
        f"Denoise {denoise_strength} for video animation from image"
    )
...
def _get_video_settings(
    ...
    motion_bucket_map = {"low": 80, "medium": 127, "high": 180}
    augmentation_map = {"low": 0.0, "medium": 0.1, "high": 0.3}
...
def _generate_video_diagnostics(settings: Dict[str, Any], intent: Dict[str, str]) -> Dict[str, str]:
    return {
        ...
        "motion_setting": f"Motion bucket ID {settings['motion_bucket_id']} for {intent.get('style','generic')} style",
        ...
        "detected_style": intent.get("style", "unspecified"),
...
def _get_negative_prompt(intent: Dict[str, str]) -> str:
    return "blurry, low quality, watermark, artifacts, bad anatomy"


def _generate_diagnostics(settings: Dict[str, Any], intent: Dict[str, str], goal: str) -> Dict[str, str]:
    return {
        "cfg_reason": f"CFG {settings['cfg_scale']} for {intent.get('style','generic')} style",
        "goal": goal,
    }
